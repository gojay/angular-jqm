...
jqmModule.directive('jqmCollapsible', function() {
    return {
        restrict: 'A',
        require: '^jqmCollapsibleSet',
        templateUrl: '../dev/templates/jqmCollapsible.html',
        scope: 'isolate',
        transclude: true,
        replace: true,
        compile: function(celement, cattr, transclude){
            ...
            return function(scope, element, attr, jqmCollapsibleSetCtrl){
                console.log(jqmCollapsibleSetCtrl);

                // var icon = jqmCollapsibleSetCtrl.icon;
                // scope.icon = icon.open;
                // scope.$watch('isCollapsed', function(opened){
                //     scope.icon = opened ? icon.open : icon.close ;
                // });
...
jqmModule.directive('jqmCollapsibleSet', function () {
    var isDef = angular.isDefined;
    var scopeSet = [];
    return {
        restrict: 'A',
        transclude: true,
        templateUrl: '../dev/templates/jqmCollapsibleSet.html',
        scope: {
            basic:'@',
            icon:'@',
            inset:'@'
    ...
    function jqmCollapsibleSetCtrl($scope, $element){
        ...
        // }
        // var icon = $scope.icon;
        // if(angular.isDefined(icon) && icon !== 'plus'){
        //     angular.extend(this.icon, {
        //         open: 'ui-icon-arrow-u',
        //         close: 'ui-icon-arrow-d'
        //     });
        // }
 ...
 *
 * @example
 <example module="jqmDialog">
 <file name="index.html">
 <div jqm-dialog>
    <div jqm-header jqm-theme="d">
        <a jqm-button jqm-theme="d" href="#/" icon="ui-icon-delete" iconpos="notext" buttonpos="left"></a>
        <h1>Dialog</h1>
    </div>
    <div class="ui-content ui-body-c">
        <h1>Delete page?</h1>
        <p>lorem ipsum dolor ist amet</p>
        <a href="#/" jqm-button jqm-theme="b">Ok</a>
        <a href="#/" jqm-button jqm-theme="c">Cancel</a>
    </div>
</div>
 </file>
 </example>

jqmModule.directive('jqmDialog', function() {
    var isdef = angular.isDefined;
    return {
        restrict: 'A',
        // replace: true,
...
jqmModule.directive('iscroll', function () {
    var isDef = angular.isDefined;
    return {
        restrict: 'EA',
        transclude: true,
        replace: true,
        templateUrl: '../dev/templates/iscroll.html',
        scope: {
            parent: '=',
            scroller: '@',
            topOffset: '@',
        ...
        link: function(scope, element, attr, iscrollableCtrl){
            ...
            if(!isDef(attr.id)) throw new Error("Attribute ID not found");
            if(!isDef(scope.parent)) throw new Error("Scope parent not found");

            scope.scroller = isDef(attr.scroller) ? attr.scroller : 'scroller' ;
            scope.pullRefresh = isDef(attr.pullRefresh) ? attr.pullRefresh : false ;
            ...
            var options = {
                useTransition : isDef(attr.transition) ? Boolean(attr.transition) : true,
                snap          : isDef(attr.snap)       ? Boolean(attr.snap) : false,
                momentum      : isDef(attr.momentum)   ? Boolean(attr.momentum) : false,
                vScrollbar    : isDef(attr.vertical)   ? Boolean(attr.vertical) : false,
                hScrollbar    : isDef(attr.horizontal) ? Boolean(attr.horizontal) : false,
                lockDirection : isDef(attr.lock)       ? Boolean(attr.lock) : true
            };

            if( isDef(attr.topOffset) ) angular.extend(options, {topOffset:scope.topOffset});
            ...
            console.log('parent', scope.parent.$parent)

            // send iscroll object to controller's parent scope
            if(!isDef(scope.parent.$parent['iscroll'])) scope.parent.$parent['iscroll'] = {};
            scope.parent.$parent['iscroll'][id] = iscroll;
